...
LOOP:   
        ...
        STMFD   SP!, {R5, PC}   /* 1> push on the stack (in memory, hence "STM") the switch state, and return address */
        B       BinaryDecoder
        @ LDR     R6, [SP,#4]!   /* 5> load to R6 the display pattern (same function as " LDMEA   SP!, {R6} ")*/
        LDMFD   SP!, {R6}       /* 5> pop the 7-segment display pattern off the stack (to processor r6) */
...
BinaryDecoder:
        LDMFD   SP!, {R5, R7}           /* 2> pop the switch value and return value off the stack (to processor at R5, R7) */
        ...
        STMFD   SP!, {R6}            /* 3> push the 7-segment display pattern onto the stack */
